...
dados$vary=dados$vprod_5; #vari?vel de interesse chamada de vary
...
#plantio=subset(dados,!duplicated(plantio),c('plantio')); #v?rios, mas tem talh?o com s? uma parcela. n?o d? pra fazer por talh?o
...
mat1=subset(calc,matgen==1);mat1;
areamat1=sum(mat1$area);areamat1;

mat2=subset(calc,matgen==2);mat2;
areamat2=sum(mat2$area);areamat2;

mat3=subset(calc,matgen==3);mat3;
areamat3=sum(mat3$area);areamat3;

mat4=subset(calc,matgen==4);mat4;
areamat4=sum(mat4$area);areamat4;

mat5=subset(calc,matgen==5);mat5;
areamat5=sum(mat5$area);areamat5;
...
#
#Volume/ha
dados$vary_ha=dados$vary*10000/dados$areaparc
View(dados)

#para calcular o volume medio m?/ha 
vmed_ha=mean(dados$vary_ha);vmed_ha
...
#?rea parcela
#N?o compensa trabalhar com parcelas com tamanhos desiguais
(areaparc = mean(dados$areaparc));


#?rea da fazenda  
talhao = subset(dados,!duplicated(plantio), c('plantio','area')); #Subset - Pegar apenas uma informa??o por parcela
View(talhao)
...
#Intervalo de Confian?a m?/parcela
cat(paste(round(ym-populacao$errounid,2),' a ',round(ym+populacao$errounid,2),'m?/parcela com 95% de confian?a',sep=''))

#Intervalo de confian?a em m?/ha
cat(paste(round(yha-erroinvha,2),' a ',round(yha+erroinvha,2),'m?/ha com 95% de confian?a',sep=''))

#Intervalo de confian?a em m?/povoamento
cat(paste(round((ytot - ytot*populacao$erroperc/100),2),
          ' a ',
          round((ytot + ytot*populacao$erroperc/100),2),
          'm?/fazenda com 95% de confian?a',sep=''));
